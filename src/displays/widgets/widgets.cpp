...
#include "../fonts/AvignonProMedium26.h"
//#include "../fonts/BahnschriftSemiCondensed13.h"
#include "../fonts/codecs.h"
#include "../../core/timekeeper.h"
#include "../../core/display.h"
//#include "../fonts/iconsweather.h"


constexpr int CAL_LEFT      = 24;
constexpr int CAL_TOP       = 138;

constexpr int VU_RECT_X    = 8;// Координата X для левого VU-метра
constexpr int VU_RECT_WIDTH    = 22;// Ширина прямоугольника VU-метра
constexpr int VU_RECT_HEIGHT   = 12;// Высота прямоугольника VU-метра
constexpr int VU_RECT_Y_LEFT       = 80;// Координата Y для VU-метров
constexpr int VU_RECT_Y_RIGHT      = VU_RECT_Y_LEFT + VU_RECT_HEIGHT +4;// Координата Y для VU-метров
...
void FillWidget::_draw(){
 ...
 dsp.drawRGBBitmap(0, 239-28, ip, 21, 24); 
 dsp.drawRGBBitmap(140, 239-28, sound, 29, 24); 
 dsp.drawRGBBitmap(230, 239-28, wifi, 34, 24); 
  }

if(display.mode() == PLAYER) {
  dsp.drawFastHLine(0, 114, 320, 0xffff); //_fgcolor
}
}
...
void VuWidget::_draw(){
  ...
  static uint16_t peakL = 0, peakR = 0;
  static uint32_t peakHoldTimeL = 0, peakHoldTimeR = 0;
  const uint32_t PEAK_HOLD_MS = 40;      // задержка перед сползанием пика, мс
  const uint8_t PEAK_FALL_SPEED = 1;     // скорость сползания пика, пикс/шаг
  const uint8_t PEAK_LINE_WIDTH = 2;     // толщина линии пика, пикс
  ...
  // --- расчет и отображение Peak Hold (режим справа налево) ---
  // Левый канал
  if(measL < peakL) {
    peakL = measL;                // пик всегда на левом краю VU (от правого края)
    peakHoldTimeL = now;
  } else if(peakL < measL) {
    // пик начинает скользить вправо только после задержки
    if(now - peakHoldTimeL > PEAK_HOLD_MS) {
      peakL = (peakL + PEAK_FALL_SPEED < measL) ? peakL + PEAK_FALL_SPEED : measL;
      if(peakL > measL) peakL = measL; // пик не может быть правее VU
    }
  } else {
    peakL = measL; // если VU догнал пик, синхронизируем
  }
  ...
  if(measR < peakR) {
    ...
    if(now - peakHoldTimeR > PEAK_HOLD_MS) {
      peakR = (peakR + PEAK_FALL_SPEED < measR) ? peakR + PEAK_FALL_SPEED : measR;
      if(peakR > measR) peakR = measR;
    }
  } else {
    peakR = measR;
  }
  ...
  uint16_t peakColor = 0xFFEC;//0xFFE0; // YELLOW
  _canvas->fillRect(0, 0, _bands.width, _bands.height * 2 + _bands.space, _bgcolor);
  for(int i = 0; i < dimension; i++){
    if(i % (dimension / _bands.perheight) == 0){
      uint16_t bandColor = (i > _bands.width - (_bands.width / _bands.perheight) * 5) ? _vumaxcolor : _vumincolor;
      _canvas->fillRect(i, 0, h, _bands.height, bandColor);
      _canvas->drawRect(i, 0, h, _bands.height, 0xffff);
      _canvas->fillRect(i, _bands.height + _bands.space, h, _bands.height, bandColor);
      _canvas->drawRect(i, _bands.height + _bands.space, h, _bands.height, 0xffff);
    }
  }
  // VU bars (справа налево)
  ...
  // --- рисуем пики отдельной линией (например, желтым) ---
  if(peakL < measL)
    _canvas->fillRect(_bands.width - peakL, 0, PEAK_LINE_WIDTH, _bands.height, peakColor);
  if(peakR < measR)
    _canvas->fillRect(_bands.width - peakR, _bands.height + _bands.space, PEAK_LINE_WIDTH, _bands.height, peakColor);
...
void VuWidget::_clear(){
  dsp.fillRect(_config.left, _config.top, _bands.width * 2 + _bands.space, _bands.height*2+_bands.space, _bgcolor);
}
#else // DSP_LCD
VuWidget::~VuWidget() { }
void VuWidget::init(WidgetConfig wconf, VUBandsConfig bands, uint16_t vumaxcolor, uint16_t vumincolor, uint16_t bgcolor) {
...
void ClockWidget::init(WidgetConfig wconf, uint16_t fgcolor, uint16_t bgcolor){
  Widget::init(wconf, fgcolor, bgcolor);
  _timeheight = _textHeight();//CHARHEIGHT * TIME_SIZE;
  ...
  #ifndef HIDE_DATE
  if(_fullclock){
    _dateheight = 2;//date size
    _clockheight = _timeheight;//+ _space  + CHARHEIGHT * _dateheight+1;//date height
  } else {
...
void ClockWidget::_getTimeBounds() {
  ...
  if(_fullclock){
    ...
    _clockwidth = _timewidth+rightside-3; // ширина виджета часов
...
void ClockWidget::_printClock(bool force){
  ...
  if(force){
    ...
    if(_fullclock){
      // lines, date & dow
//################  вывод иконки календаря  ###################
if(!config.isScreensaver) {

 if(network.timeinfo.tm_min % 1 == 0) timekeeper.PrintIcon();
 if(network.timeinfo.tm_min % 2 == 0) {
 //dsp.fillRect(20, 122, 64, 30, 0x0000); 
dsp.drawRGBBitmap(CAL_LEFT, CAL_TOP,fon2, 76, 82);
dsp.setFont(&BahnschriftSemiCondensed13);
dsp.setTextSize(1);

dsp.setTextColor(0xD69A);//цвет месяца
const char* monthStr = utf8Rus(LANG::mnths[network.timeinfo.tm_mon], true);
    uint16_t monthWidth = strlen(monthStr) * 9/2; //рассчет примерной ширины
    dsp.setCursor(CAL_LEFT + 36 - monthWidth, CAL_TOP +28);  
    dsp.print(monthStr);//месяц

    // цвет для дня недели/числа: воскресенье — красный, иначе — чёрный
    uint16_t dayColor = (network.timeinfo.tm_wday == 0) ? 0xffff : 0xffee; // RGB565 red / black

const char* dowStr = utf8Rus(LANG::dow[network.timeinfo.tm_wday], false);
    uint16_t dowWidth = strlen(dowStr) * 4; //рассчет примерной ширины
    dsp.setTextColor(dayColor);
    dsp.setCursor(CAL_LEFT + 38 - dowWidth, CAL_TOP + 77);
...
dsp.setTextSize(1);
dsp.setTextColor(dayColor);
sprintf(_tmp, "%2d", network.timeinfo.tm_mday);
dsp.setCursor(CAL_LEFT+38-18*strlen(_tmp)/2, CAL_TOP+61);
...
//dsp.drawRGBBitmap(8, 150, img_01d, 64, 37);
      ...
      if(fullClockOnScreensaver){
        //gfx.drawFastVLine(_linesleft-7, _top()-_timeheight, _timeheight, config.theme.div);
       //gfx.drawFastHLine(_linesleft-7, _top()-(_timeheight)/2+27, CHARWIDTH * _superfont * 2 + _space+7, config.theme.div);
       /* gfx.setFont(&BahnschriftSemiCondensed14);
        gfx.setTextSize(1);
        gfx.setCursor(_linesleft+_space-182, _top() + _space+15);
        gfx.setTextColor(config.theme.dow, config.theme.background);
        gfx.print(utf8Rus(LANG::dowf[network.timeinfo.tm_wday], false));
        sprintf(_tmp, "%02d.%s.%d", network.timeinfo.tm_mday,LANG::mnths[network.timeinfo.tm_mon], network.timeinfo.tm_year+1900);
        #ifndef HIDE_DATE
        strlcpy(_datebuf, utf8Rus(_tmp, true), sizeof(_datebuf));
        uint16_t _datewidth = strlen(_datebuf) * 9/2*_dateheight;
        gfx.setTextSize(1);
        #if DSP_MODEL==DSP_GC9A01A
        gfx.setCursor((dsp.width()-_datewidth)/2, _top() + _space);
        #else
        gfx.setCursor(_left()+_clockwidth-_datewidth+8, _top() + _space+15);
        #endif
  ...
  if(_fullclock || _superfont>0){
    gfx.setFont(&AvignonProMedium26);
    gfx.setTextSize(1);
    //dsp.fillRect(_linesleft+_space+1, _top()-_timeheight+20, 32, 20, config.theme.background+20);
    if(!_fullclock){
      #ifndef CLOCKFONT5x7
      ...
      gfx.setCursor(_linesleft+_space-8, _top());
      gfx.print(";;");
      gfx.setCursor(_linesleft+_space-8, _top());
    }
...
void BitrateWidget::_draw(){
  ...
  if(config.store.vumeter) {
    dsp.drawRect(VU_RECT_X, VU_RECT_Y_LEFT, VU_RECT_WIDTH, VU_RECT_HEIGHT, 0xffff);
    dsp.drawRect(VU_RECT_X, VU_RECT_Y_RIGHT, VU_RECT_WIDTH, VU_RECT_HEIGHT, 0xffff);
   
     dsp.setFont();
    dsp.setTextSize(1);
   
    dsp .setTextColor(0xFFEC, _bgcolor);
    
    dsp.setCursor(VU_RECT_X + 9, VU_RECT_Y_LEFT + 2);
    dsp.print("L");
    
    dsp.setCursor(VU_RECT_X + 9, VU_RECT_Y_RIGHT + 2);
    dsp.print("R");
  ...
  if(_bitrate < 999)  snprintf(_buf, 6, "%dk", _bitrate);
  else {
    float _br = (float)_bitrate / 1000;
    snprintf(_buf, 6, "%.2fM", _br);
  }
  dsp.drawRect(_config.left, _config.top-_dimension/2+4, _dimension, _dimension/2-4, _fgcolor);
  dsp.setCursor(_config.left + 27 - 8*strlen(_buf)/2, _config.top -6);
  dsp.print(_buf);
  //dsp.print("K");
  ...
  switch(_format){
    case BF_MP3:  dsp.drawRGBBitmap(_config.left, _config.top, mp3, 54, 25); break;
    case BF_AAC:  dsp.drawRGBBitmap(_config.left, _config.top, aac, 54, 25);  break;
    case BF_FLAC: dsp.drawRGBBitmap(_config.left, _config.top, flac, 54, 25); break;
    case BF_OGG:  dsp.drawRGBBitmap(_config.left, _config.top, ogg, 54, 25); break;
    case BF_OPUS:  dsp.drawRGBBitmap(_config.left, _config.top, opus, 54, 25); break;
    case BF_VOR:  dsp.drawRGBBitmap(_config.left, _config.top, vor, 54, 25); break;
...
void BitrateWidget::_clear() {
  dsp.fillRect(_config.left,_config.top-_dimension/2+4,  54, 55, _bgcolor);
  dsp.fillRect(VU_RECT_X, VU_RECT_Y_LEFT, VU_RECT_WIDTH, VU_RECT_HEIGHT*2+5,  _bgcolor);
}
