...
#include "../displays/fonts/AvignonProMedium22.h"
#ifdef USE_NEXTION
...
constexpr int WTR_LEFT      = 30; 
constexpr int WTR_TOP       = 151;  
  ...
  weatherClient->onConnect([](void * arg, AsyncClient * client){
    ...
    client->onData([](void * arg, AsyncClient * c, void * data, size_t len){
      ...
      if (bodyStart != NULL) {
        ...
        /* parse it */
        char *cursor;
        char desc[120],  station[64];// icon[5],
        float  tempfl, wind_speed;
        ...
        press = press * 1.00526;//press = press / 1.333; // convert hPa to mmHg
        ...
        sprintf(timekeeper.weatherBuf, weatherFmt, station, tempf, press, hum);
        #else
          #if EXT_WEATHER
            sprintf(timekeeper.weatherBuf, LANG::weatherFmt, desc, tempf, tempfl, press, hum, wind_speed, LANG::wind[(int)(wind_deg/22.5)], station);
          #else
            sprintf(timekeeper.weatherBuf, LANG::weatherFmt, station, desc, tempf, press, hum);
          #endif
        #endif
...
void TimeKeeper::PrintIcon() {
   ...
   dsp.fillRect(WTR_LEFT-6, 138, 76, 82, 0x0000);
   dsp.drawRGBBitmap(WTR_LEFT, WTR_TOP, yo, 64, 37);
   return;

   }

dsp.drawRGBBitmap(WTR_LEFT-6, WTR_TOP-13, fon, 76, 82); 
dsp.setFont(&AvignonProMedium22);
dsp.setTextSize(1);
dsp.setTextColor(0xFFee, 0x0000); 


char tempStr[10];
sprintf(tempStr, "%.0f<", tempf);
dsp.setCursor(WTR_LEFT + 30 - (strlen(tempStr)*5), WTR_TOP+64);
dsp.print(tempStr);
dsp.setFont();


  if(strstr(icon,"01d")!=NULL)      dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, img_01d, 36, 36);
  else if(strstr(icon,"01n")!=NULL) dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, img_01n, 36, 36);
  else if(strstr(icon,"02d")!=NULL) dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, img_02d, 36, 36);
  else if(strstr(icon,"02n")!=NULL) dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, img_02n, 36, 36);
  else if(strstr(icon,"03d")!=NULL) dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, img_03d, 36, 36);
  else if(strstr(icon,"03n")!=NULL) dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, img_03n, 36, 36);
  else if(strstr(icon,"04d")!=NULL) dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, img_04d, 36, 36);
  else if(strstr(icon,"04n")!=NULL) dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, img_04n, 36, 36);
  else if(strstr(icon,"09d")!=NULL) dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, img_09d, 36, 36);
  else if(strstr(icon,"09n")!=NULL) dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, img_09n, 36, 36);
  else if(strstr(icon,"10d")!=NULL) dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, img_10d, 36, 36);
  else if(strstr(icon,"10n")!=NULL) dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, img_10n, 36, 36);
  else if(strstr(icon,"11d")!=NULL) dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, img_11d, 36, 36);
  else if(strstr(icon,"11n")!=NULL) dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, img_11n, 36, 36);
  else if(strstr(icon,"13d")!=NULL) dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, img_13d, 36, 36);
  else if(strstr(icon,"13n")!=NULL) dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, img_13n, 36, 36);
  else if(strstr(icon,"50d")!=NULL) dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, img_50d, 36, 36);
  else if(strstr(icon,"50n")!=NULL) dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, img_50n, 36, 36);

  else                             dsp.drawRGBBitmap(WTR_LEFT+14, WTR_TOP, weather, 36, 36);
  
 
  }

  void TimeKeeper::PrintLogo() {
/*
    if (!config.store.showweather) {
   dsp.drawRGBBitmap(WTR_LEFT, WTR_TOP, eradio, 64, 37);
   return;
    }
  dsp.drawRGBBitmap(WTR_LEFT, WTR_TOP, weather, 64, 37);
