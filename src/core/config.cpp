...
bool Config::prepareForPlaying(uint16_t stationId){
  ...
  // Для SD-карты сразу показываем имя записи из playlist.csv — не использовать заглушку
  if (getMode() == PM_WEB) {
...
void Config::resetSystem(const char *val, uint8_t clientId){
  ...
  if (strcmp(val, "timezone") == 0) {
    ...
    saveValue(store.sntp2, "0.eu.pool.ntp.org", 35);
  ...
  if (strcmp(val, "weather") == 0) {
    ...
    saveValue(store.weatherlat, "54.6675", 10, false);
    saveValue(store.weatherlon, "25.0966", 10, false);
...
void Config::setDefaults() {
  ...
  strlcpy(store.sntp2,"1.eu.pool.ntp.org", 35);
  store.showweather=false;
  strlcpy(store.weatherlat,"54.6675", 10);
  strlcpy(store.weatherlon,"25.0966", 10);
